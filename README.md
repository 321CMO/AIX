...
### 1. 创建目录
    ...
    mkdir -p ~/Documents/Xray-macos-64
    cd ~/Documents/Xray-macos-64

**重要**：文件夹名称必须为 `Xray-macos-64`，脚本会自动扫描该目录下的配置文件。

### 2. 下载 Xray 核心

**macOS 10.14 用户**（推荐 v23.3.1）：

    curl -LO https://github.com/XTLS/Xray-core/releases/download/v23.3.1/Xray-macos-64.zip
...
将你的节点配置文件（如 `config-hkt.json`、`config-jp.json`）放入 `~/Documents/Xray-macos-64/` 目录。
...
## 📁 目录结构
    ...
    ├── xray                    # Xray 核心文件（必须有）
    ├── config-hkt.json        # 香港节点配置
    ├── config-jp.json         # 日本节点配置
    └── ...                    # 其他节点配置
...
**重要说明**：
- `Xray-macos-64` 文件夹名称**必须保持一致**
- 所有节点配置文件必须以 `config-` 开头，`.json` 结尾
- 示例：`config-hkt.json` → 节点名显示为 `hkt`
...
A: 确保 `xray` 文件在 `~/Documents/Xray-macos-64/` 目录且有执行权限：
